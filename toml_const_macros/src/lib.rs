...
struct MacroInvocation<T> {
    runtime_path: syn::Path,
    input: T,
}

impl<T: Parse> Parse for MacroInvocation<T> {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let runtime_path = input.parse()?;
        input.parse::<Token![;]>()?;
...
/// Implementation of [`toml_const::toml_const`].
#[doc(hidden)]
#[proc_macro]
pub fn __toml_const(input: pm::TokenStream) -> pm::TokenStream {
    let MacroInvocation {
        runtime_path,
        input,
    }: MacroInvocation<MultipleMacroInput> = parse_macro_input!(input);
    ...
    let inner_calls = input
        ...
        .map(|i| {
            ...
            quote! {
                #runtime_path::__toml_const_inner! {
...
/// Implementation of [`toml_const::toml_const_ws`].
#[doc(hidden)]
#[proc_macro]
pub fn __toml_const_ws(input: pm::TokenStream) -> pm::TokenStream {
    let MacroInvocation {
        runtime_path,
        input,
    }: MacroInvocation<MultipleMacroInput> = parse_macro_input!(input);
...
/// Inner method call generated by the public macro implementations.
#[doc(hidden)]
#[proc_macro]
pub fn __toml_const_inner(input: pm::TokenStream) -> pm::TokenStream {
    let MacroInvocation {
        runtime_path,
        input,
    }: MacroInvocation<MacroInput> = parse_macro_input!(input);
    ...
    let table_definitions = toml_val_table.definition(
